...
public class SimpleJWT {
	...
	public static void main(String[] args) throws Exception {
		// 1. 生成簽名密鑰
		// JWK:　產生簽名用ㄉ密鑰（32 bytes）
		String signingSecret = KeyUtil.generateSecret(32);
		System.out.println("密鑰" + signingSecret);
		
		// 2. 創建 JWT 聲明(claim)
		// JWT: 建立資料主體(之後要進行簽名ㄉ部分)
		JWTClaimsSet claimsSet = new JWTClaimsSet.Builder(null)
					.subject("報名java")  // 主題
					.issuer("http://edu/cdri/org/tw")   // 發行單位
					.claim("name", "John")				// 額外資料
					.claim("phine", "0912345678")		// 額外資料
					.claim("email", "john@gmail.com")	// 額外資料
					.build();
