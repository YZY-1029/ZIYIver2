...
public class Coffee {
	...
	//可以儲存所有咖啡規則的清單
	public static final List<CoffeeRule> rules = new ArrayList<>();
	...
	static{
		...
		rules.add(new CoffeeRule("標準卡布奇諾", "牛奶與咖啡的比例恰到好處，典型的卡布奇諾。", (milk, coffee) -> milk.doubleValue() == coffee.doubleValue()));
		//rules.add(new CoffeeRule("標準卡布奇諾", "牛奶與咖啡的比例恰到好處，典型的卡布奇諾。", (milk, coffee) -> milk.equals(coffee)));
		rules.add(new CoffeeRule("濃縮咖啡", "咖啡的比例遠高於牛奶，口感濃烈，偏向純咖啡。", (milk, coffee) -> milk * 3 <= coffee));
	...
	public String getCoffeeType() {
		return rules.stream()
					.filter(rule -> rule.matches(milk, coffee)) // 找出符合條件規則
					.findFirst()								//取的第一筆匹配的	
					//.map(CoffeeRule::getResult)					//傳喚成字串
